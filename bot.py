...
class WindowBot:
    ...
    def setup_handlers(self):
        ...
        async def process_text_message(message: Message, state: FSMContext):
            ...
            # Проверяем, является ли вопрос сложным (сравнение, отличие и т.д.)
            if self.db.is_complex_question(query):
                complex_response = (
                    "К сожалению, я не могу дать детальный ответ на такой сложный вопрос.\n\n"
                    "Для получения подробной консультации рекомендую:\n"
                    "• 📋 Посмотреть раздел /faq с часто задаваемыми вопросами\n"
                    "• 📅 Записаться на бесплатный замер (/book) - наш специалист даст детальную консультацию и ответит на все вопросы\n"
                    "• 💬 Задать более простой вопрос, на который я смогу ответить"
                )
